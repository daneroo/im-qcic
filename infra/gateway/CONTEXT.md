...
**Note — validation needed for v2/status's Dockerfile (#227)**: `v2/` is a Bun workspace, so `status`'s Dockerfile build context must be the workspace root (`v2/`), not `v2/apps/status/` itself — `bun install` needs the root `package.json`/`bun.lock`/sibling packages to resolve workspace links. The Dockerfile can still live at `v2/apps/status/Dockerfile` (build with `-f apps/status/Dockerfile`, context `v2/`); Gateway's `docker-compose.yaml` `build:` field for `status` will need to change from a plain string to an object with separate `context`/`dockerfile` keys (standard Compose syntax, not Bun-specific). An alternative — Docker BuildKit's "multiple build contexts" (`additional_contexts`, Compose ≥2.17) — would let the Dockerfile's own context stay narrow instead, but that needs Gateway's actual Docker/Compose version confirmed first (not yet checked — SSH to Gateway wasn't reachable from within a Claude Code sandbox; check `docker compose version` on Gateway directly). Until confirmed, default to the plain context+dockerfile-keys approach, which works on any Compose version.
