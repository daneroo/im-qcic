...
broken. Two gitignored files are needed at the workspace-root
...
comment for why the container-side path is `/usr/src/app/infra/credentials/`):

```sh
docker run -d --name qcic-status -p 8000:8000 \
  -v "$(pwd)/infra/credentials/credentials.mysql.json:/usr/src/app/infra/credentials/credentials.mysql.json:ro" \
  -v "$(pwd)/infra/credentials/credentials.loggly.json:/usr/src/app/infra/credentials/credentials.loggly.json:ro" \
  qcic-status:latest
```
